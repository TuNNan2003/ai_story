...
- 我的故事列表增加删除和修改按钮
    - 可以通过点击修改按钮，拉起编辑界面，修改标题或内容
- 编辑界面修改
    - 编辑界面需要变得更大，覆盖70%右侧的大模型对话页面
...
当我把网页开放给外网使用时发现，目前系统没有用户ID的概念，请修复这一点
为每个用户生成一个唯一的ID，且用户多次打开网页都一样

## v0.4.5 Features

增加功能，当点击我的故事列表内的某一行时，可以拉起一个展示故事的页面，渲染markdown格式保存的故事并展示，没有编辑功能
...
- 在新对话按钮右侧增加一个开关，开关打开后进入灵感模式

- 灵感模式时，左侧的历史对话和我的故事栏被替换成「我的创作」栏

- AI对话初始界面保持不变，在开启对话后，对话页面部分分成左右两部分，参考gemini的deepResearch模式
    - 左边展示对话记录，右边将AI返回的内容以markdown格式渲染
    - 左侧用户发出请求后，在右侧以打字机模式渲染AI返回的markdown内容，AI全部返回后，在左侧用户请求下面展示一个AI已经返回内容的标识，提示用户可以开启下一次对话
- 页面最右侧新增一个文档列表，这个文档列表是和我的创作关联的，一个创作包含了多个文档
    - 文档列表最上方有一个新增按钮，点击后弹出一个输入框，支持用户自定义输入文档并命名保存
    - 文档列表有重命名和删除功能
- 灵感模式下AI对话框内增加一个「在原文上修改」的选项，打开此选项会提示AI这次返回需要在之前文档内容上修改

## v1.1 Features

- 灵感模式下用户输入问题后，不在左侧展示AI返回的内容，而是在右侧以打字机模式展示AI返回
    - AI内容全部返回后，在左侧对话列表内，用户的问题下方，展示一个"我已完成创作，你可以提出后续问题或者要求进行改动"，在这段字下方，展示一个可以点击的模块，点击后可以展示此次AI返回的文档


## v1.2 Features

- 点击查看此次创作内容不会拉起一个浮层来展示文档，而是在右侧展示该文档内容

## v1.3 Features

- 在灵感模式内与AI进行对话不会保存在对话模块内，而是要保存在创作模块内

- 后台提供创作模块，支持对创作的增加、删除、重命名、查询


## v2 长篇故事写作优化 Features

RAG (Retrieval-Augmented Generation) 的核心思想是：不要把所有记忆都塞给AI，而是在AI需要时，帮它“检索”出最相关的记忆。

工作流程：

“记忆”入库（Indexing）：

你把所有的小说内容、大纲、人物卡、世界观设定，全部“切片”成小块（Chunks）。

将这些“小块”通过一个模型（Embedding Model）转换成数学向量（Vectors）。

把这些向量存入一个向量数据库（Vector Database）。这个数据库就像AI的“长期记忆库”。

“记忆”检索（Retrieval）：

当AI准备写新内容时（例如，用户输入：“写爱丽丝和鲍勃在图书馆对峙”）。

你的应用首先拿着这个指令（“爱丽丝和鲍勃在图书馆”）去向量数据库里搜索。
...
“爱丽丝和鲍勃上次争吵的摘要（发生在第3章）”

“图书馆的场景描述（在第5章出现过）”

“关于‘神秘之书’的伏笔（鲍勃正在寻找它）”

“记忆”增强（Augmented Generation）：

你的应用把这些检索到的“记忆片段”和用户的指令拼接成一个新的、信息量极大的提示词。

将这个“增强后的提示词”发送给LLM。

结果： AI在下笔前，就拿到了所有相关的背景资料，它“忘记”的概率就大大降低了。

如何实现： 你需要使用 LangChain 或 LlamaIndex 这样的框架，它们专门用于简化RAG流程，并配合一个向量数据库（如 ChromaDB, Pinecone 或 FAISS）。


## v2.1 Features

在灵感模式下的右侧展示AI故事的部分，底部增加一栏，有保存，编辑，退出三个按钮
待AI的响应全部接收到，或者用户点击进入一个之前的文档时，可以点击这三个按钮
点击后，可以在右侧展示故事的地方对文档内容进行编辑和保存
